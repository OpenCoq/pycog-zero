    ...
    def _forward_chain_internal(self, source_atoms, max_steps, profiler):
        ...
        if self.rule_base and TORCHPLN_AVAILABLE:
            try:
                ...
                result = self.forward_chainer.get_results().out if self.forward_chainer.get_results() else []
            except Exception as e:
                print(f"⚠️ TorchPLN forward chaining warning: {e}")
                result = self._enhanced_fallback_forward_chain(source_atoms, max_steps)
    ...
    def backward_chain(self, target_atoms, max_steps=10):
        ...
        if self.rule_base and TORCHPLN_AVAILABLE:
            ...
            try:
                ...
                result = self._enhanced_fallback_backward_chain(target_atoms, max_steps)
        else:
            # Enhanced fallback implementation with probabilistic reasoning
            result = self._enhanced_fallback_backward_chain(target_atoms, max_steps)
    ...
    async def _reason_with_atomspace(self, query: str, **kwargs):
        ...
        try:
            ...
            return Response(
                ...
                data={'error': str(e), 'status': 'error', 'fallback_available': True},
                break_loop=False
            )
